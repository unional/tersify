...
describe('function', () => {
	...
	test('with variable declaration', () => {
		...
		expect(tersify(subject, { maxLength: 59 })).toBe(
			`fn() { const date = 2020-05-14T11:45:27.234Z; return date }`
		)
		expect(tersify(subject, { maxLength: 58 })).toBe(
			`fn() { const date = 2020-05-14T11:45:27.234Z; return ... }`
		)
	...
	test('with this expression', () => {
		const subject = function withThis(this: any) {
			return this
		}
	...
	test('with spread', () => {
		const subject = function spread(x) {
			console.info(...x)
		}
	...
	test('returns template literal', () => {
		const subject = function templateLiteral() {
			return `abc`
		}
...
describe('arrow function', () => {
	...
	test('with variable declaration', () => {
		...
		expect(tersify(subject, { maxLength: 60 })).toBe(
			`() => { const date = 2020-05-14T11:45:27.234Z; return date }`
		)
		expect(tersify(subject, { maxLength: 59 })).toBe(
			`() => { const date = 2020-05-14T11:45:27.234Z; return ... }`
		)
...
describe('object', () => {
	...
	test('trimming object with arrow function', () => {
		expect(tersify({ a: () => false, c: { b: 1, c: 'c' }, d: true }, { maxLength: 20 })).toBe(
			`{ a: () => fals... }`
		)
	})
...
describe('array', () => {
	...
	test('trim the entry when exceed maxLength', () => {
		expect(tersify(['123456789012345678901234567890'], { maxLength: 34 })).toBe(
			`['123456789012345678901234567890']`
		)
		expect(tersify(['123456789012345678901234567890'], { maxLength: 33 })).toBe(
			`['123456789012345678901234567...]`
		)
		expect(tersify(['123456789012345678901234567890'], { maxLength: 32 })).toBe(
	...
	test('use tersify method for each element in array', () => {
		...
		expect(tersify([a, b, { c: 3 }, undefined, null, 1, 'a'])).toBe(
